...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT)) {
            ...
            std::vector<float> targetXValues = {
            ...
            for (int i = 0; i < targetXValues.size(); i++) {
                float zoomLowerBound = 0.36f - i * 0.01f - 0.001f;
                float zoomUpperBound = 0.36f - i * 0.01f + 0.001f;
                if (camera.zoom > zoomLowerBound && camera.zoom < zoomUpperBound && camera.target.x > targetXValues[i]) {
                    camera.target.x = targetXValues[i];
                    break;
                }
            }


            std::vector<float> targetYValues = {
             19253.25f, 19104.75f, 18938.273438f, 18765.167969f, 18591.777344f,
             18396.193359f, 18189.298828f, 17976.751853f, 17764.203878f, 17469.074219f,
             17234.947266f, 16931.408203f, 16607.429688f, 16250.011719f, 15873.304688f,
             15479.590820f, 14998.400391f, 14502.624023f, 13959.341797f, 13323.376953f,
            ...
            for (int i = 0; i < targetYValues.size(); i++) {
                float zoomLowerBound = 0.36f - i * 0.01f - 0.001f;
                float zoomUpperBound = 0.36f - i * 0.01f + 0.001f;
                if (camera.zoom > zoomLowerBound && camera.zoom < zoomUpperBound && camera.target.y > targetYValues[i]) {
                    camera.target.y = targetYValues[i];
                    break;
                }
            }
        }
